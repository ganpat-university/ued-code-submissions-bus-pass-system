...
@WebServlet(urlPatterns="/renewpass")
public class renewpass extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String pass_id = req.getParameter("passid");
		...
		try {
			...
			PreparedStatement prp=con.prepareStatement("UPDATE pass_details SET fullname=?, dob=?, email=?, mobileno=?, gender=?, occupation=?, passtype=?, category=?, user_id=?, root=?, addline1=?, orgname=?, orgaddress=?, education=? WHERE pass_id=?;");
